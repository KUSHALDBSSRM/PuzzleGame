...
public class PuzzleBoard {
    ...
    PuzzleBoard(Bitmap bitmap, int parentWidth) {
        ...
        int twiBaseW = baseW * 2;
        tiles = new ArrayList();
        tiles.add(new PuzzleTile(Bitmap.createBitmap(scaleBitmap, 0, 0 ,baseW ,baseW),0));
        tiles.add(new PuzzleTile(Bitmap.createBitmap(scaleBitmap, baseW, 0 ,baseW ,baseW),1));
        tiles.add(new PuzzleTile(Bitmap.createBitmap(scaleBitmap, twiBaseW, 0 ,baseW ,baseW),2));
        tiles.add(new PuzzleTile(Bitmap.createBitmap(scaleBitmap, 0, baseW ,baseW ,baseW),3));
        tiles.add(new PuzzleTile(Bitmap.createBitmap(scaleBitmap, baseW ,baseW,baseW ,baseW),4));
        tiles.add(new PuzzleTile(Bitmap.createBitmap(scaleBitmap, twiBaseW, baseW ,baseW ,baseW),5));
        tiles.add(new PuzzleTile(Bitmap.createBitmap(scaleBitmap, 0, twiBaseW ,baseW ,baseW),6));
        tiles.add(new PuzzleTile(Bitmap.createBitmap(scaleBitmap, baseW,twiBaseW ,baseW ,baseW),7));
        tiles.add(null);  
    }
    ...
    public ArrayList<PuzzleBoard> neighbours() {
        ...
        for (int[] i : NEIGHBOUR_COORDS) {
            int neighbourX = emptyTileX + i[0];
            int neighbourY = emptyTileY + i[1];
